...
if "model" in st.session_state:
    ...
    if st.button("Predecir"):
        ...
        scatter = alt.Chart(st.session_state.edited_df).mark_circle(size=50, opacity=0.6).encode(
            x=col_x, y=col_y
        )
        line = alt.Chart(st.session_state.grid).mark_line().encode(x=col_x, y=col_y)
        dot = alt.Chart(new_point).mark_point(size=120, shape="diamond").encode(x=col_x, y=col_y)
